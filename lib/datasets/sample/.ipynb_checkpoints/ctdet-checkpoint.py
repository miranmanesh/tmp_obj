...
class CTDetDataset(data.Dataset):
  ...
  def __getitem__(self, index):
    ...
    curImg = self.coco.imgs[img_id]
    imageSize = (curImg['height'], curImg['width'])
    segMapsize = (num_classes,curImg['height'], curImg['width'])
    segMapTotal = np.zeros(segMapsize, dtype = bool)
    ctMapTotal = np.zeros (segMapsize, dtype = bool)
    widthMapTotal = np.zeros(segMapsize, dtype = np.uint8)
    heightMapTotal= np.zeros(segMapsize, dtype = np.uint8)
      ...
      if h > 0 and w > 0:
        ...
        for i in range(-sq, sq):
          for j in range(-sq, sq):
            ctMapTotal[cls_id,ct_int[0] + i, ct_int[1] + j] = True
            widthMapTotal[cls_id, ct_int[0] + i, ct_int[1] + j] = w
            heightMapTotal[cls_id , ct_int[0] + i, ct_int[1] + j] = h
    ...
    ret = {'input':org_img.astype(np.float32) , 'gt_segmap':segMapTotal.astype(np.float32), 'gt_ctmap': ctMapTotal.astype(np.float32), 'gt_widmap':widthMapTotal.astype(np.float32), 'gt_heimap':heightMapTotal.astype(np.float32)}
    #return ret
