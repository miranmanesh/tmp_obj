...
class CTDetDataset(data.Dataset):
  ...
  def __getitem__(self, index):
    ...
    #newimg = cv2.resize(img, (int(256), int(256)))
    ...
    segMapTotal = np.zeros((num_classes,output_h, output_w), dtype=np.uint8)
    ...
    ret = {'input': inp, 'hm': hm, 'reg_mask': reg_mask, 'ind': ind, 'wh': wh, 'gt_segmap':segMapTotal.astype(np.float32)}
    if self.opt.dense_wh:
