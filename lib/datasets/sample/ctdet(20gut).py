...
class CTDetDataset(data.Dataset):
  ...
  def __getitem__(self, index):
    ...
    ctMapTotal = np.zeros((num_classes, output_h, output_w), dtype=np.float32)
    ...
    #'gt_widmap':widthMapTotal.astype(np.uint8), 'gt_heimap':heightMapTotal.astype(np.uint8),
    ret = {'input':inp , 'gt_segmap':segMapTotal.astype(np.float32), 'gt_ctmap': ctMapTotal.astype(np.float32), \
           'ind': ind, 'wh': wh, 'reg_mask': reg_mask}
