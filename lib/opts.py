...
class opts(object):
  def __init__(self):
    ...
    self.parser.add_argument('--shape_param', default='',
                             help='which paramter in shapes')

    self.parser.add_argument('--shape_param_value', default='',
                             help='which paramter value in shapes')
  ...
  def init(self, args=''):
    ...
    num_classes = {'shapes': 3, 'coco': 80, 'markable':40}
    default_resolutions = {'shapes':[128, 128], 'coco':[512, 512], 'markable':[512, 512]} 

    opt = self.parse(args)
    import ipdb; ipdb.set_trace()    
    default_dataset_info['ctdet']['dataset'] = opt.dataset
    default_dataset_info['ctdet']['num_classes'] = num_classes[opt.dataset]
    default_dataset_info['ctdet']['default_resolution'] = default_resolutions[opt.dataset]
