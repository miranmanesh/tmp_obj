...
def _nms(heat, kernel=3):
    ...
    #hmax = nn.functional.avg_pool2d(heat, (kernel, kernel), stride=1, padding=pad, ceil_mode=True)
...
def ctdet_decode(heat, wh,  cat_spec_wh=False, K=100): #reg=None,
    batch, cat, height, width = heat.size()
...
def Maskct_decode(pred_masks, pred_centers, pred_widths, pred_heights, K=10):
    import pdb
    pdb.set_trace()
    batch, cat, height, width = pred_masks.shape
    ...
    for i in range(5):  # n_classes
        ...
        K = ncomponents
        cls = torch.Tensor(batch, K, 1)
        # scores, inds, clses, ys, xs = _topk(pred_masks, K=ncomponents)
